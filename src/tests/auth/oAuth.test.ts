...
describe('GET /oAuth', () => {
  ...
  it('Correct google address => status 200 / auth cookie', async () => {
    ...
    expect(GetUser).toHaveBeenNthCalledWith(1, { email: 'providerAccount@gmail.com', oAuthAccount: true }, ['id', 'role']);
  ...
  it('Correct new google address => status 200 / auth cookie', async () => {
    ...
    expect(response.status).toBe(200);
    expect(GetUser).toHaveBeenNthCalledWith(1, { email: 'NewproviderAccount@gmail.com', oAuthAccount: true }, ['id', 'role']);
