...
const validator = ({ body, params, query })=>{
    const validator = _yup.object().shape(_object_spread({}, body ? {
        body: _yup.object().shape(body)
    } : {}, query ? {
        query: _yup.object().shape(query)
    } : {}, params ? {
        params: _yup.object().shape(params)
    } : {}));
    return async (ctx)=>{
        const { req, next } = ctx;
        try {
            ...
            if (err instanceof _yup.ValidationError) {
                throw new _exceptions.InvalidArgumentError(err.errors);
            }
